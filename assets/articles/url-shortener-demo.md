...
subtitle: "把前三篇講的設計寫成可以實際執行的 code。"
...
`RETURNING` 是原子操作，多台 server 同時呼叫不會拿到重複的 ID。拿到數字後 base62 encode 成 short_key。

選 counter 不選 hash，是因為 counter 不需要碰撞檢查。Bloom filter 在這裡用來過濾**不存在的 key**（cache penetration），不是檢查 hash 碰撞。
...
5 個 hash function 是用同一個 key 加不同 seed 算出來的。每個 key 分到大約 10 bits 的話，5 個 hash 的 false positive rate 約 1%；bitset 開得越小，每個 key 分到的 bits 越少，這個比例就越高。
...
main loop: 累積到 500 筆 or 2 秒到時 → PrepareBatch → Append × N → Send
```

ClickHouse 用 native binary protocol（TCP :9000），不是 SQL text。`PrepareBatch` 發一次 INSERT header，`Append` 每筆 row 串流進去，`Send` 才真的送出。一批只送一次，所以比逐行 INSERT 快很多。
...
| Bloom filter | Redis bitset 自己實作 | RedisBloom module |
