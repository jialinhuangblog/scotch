...
updated: 2026-06-08
revisions: 1
subtitle: "同樣叫「複製」，底下有三種架構，各自給的一致性保證不一樣。"
chapter: "storage"
...
直覺的解法是把資料[複製](chunk://replication)到多台機器。一台掛了，其他的還有。

但「複製」背後其實有三種完全不同的架構，寫入、讀取、節點掛掉時的處理方式都不一樣。
...
寫入只走 leader。Leader 寫完可以馬上回覆 client（非同步複製），也可以等 follower 確認（同步複製）。大多數系統預設非同步。這樣比較快，但要是 leader 掛了，最近幾筆還沒傳到 follower 的寫入就會丟掉。

### 讀取怎麼走

讀 leader 一定拿到最新值。讀 follower 可能拿到舊值（replication lag）。

Aurora 提供兩個 endpoint：writer endpoint 連到 leader，reader endpoint 連到 follower。Application 自己決定每個 query 走哪個。
...
etcd、TiKV 用 Raft，ZooKeeper 用的是類似的 ZAB。
...
這是跟架構一的差別。Leader-Follower 預設用非同步複製，leader 寫完就回覆 client，不等 follower。Raft 的 leader 要等多數確認才回覆。

### 讀取怎麼走

所有節點的 log 順序完全一致。但 follower 可能還沒套用最新 commit 的那幾筆，所以 etcd 預設的讀取是線性一致的（linearizable read）：回應之前先跟 leader 確認最新的 commit 位置，確定自己已經追上才回。
...
只要掛掉的節點過半（3 個裡掛 2 個），叢集就停止服務。湊不到多數確認，寫入就無法 commit，Raft 的設計是這時停止回應，而不是回傳可能過時的資料。
...
etcd 和 Redis 畫出來的架構圖一模一樣：一個 leader，多個 follower。
...
k8s 用 etcd 存叢集狀態（絕對不能丟），Redis 拿來做 cache（丟了頂多 cache miss，再查一次）。
...
Cassandra、Riak 用這個架構，設計來自 Amazon 2007 年的 Dynamo 論文。現在 AWS 上的 DynamoDB 服務名字相近，但每個 partition 內部其實有 leader，不算 leaderless。
...
W 和 R 是兩個可以自己調的設定：調高（趨近 W=R=N）更偏一致，讀得到剛寫的；調低更偏快、偏可用。但再怎麼調，也給不了 consensus 那種強一致，這是 leaderless 架構本身的限制。
...
寫的當下各節點各自獨立，事後靠背景機制慢慢收斂到一致：

- **gossip**：節點之間定期交換狀態
- **read-repair**：讀取時發現副本不一致，就把較新的版本補過去
- **hinted handoff**：目標節點掛掉時，由別的節點先代收寫入，等它回來再轉交
...
所以 MySQL、PostgreSQL、Aurora 都是 Leader-Follower：所有寫入經過同一個 leader，leader 負責 transaction 的順序。Leaderless 用在不需要 transaction 的場景，例如用 Cassandra 存 log。
...
|---|---|---|---|
| 代表產品 | MySQL, Aurora, MongoDB | etcd, ZooKeeper, TiKV | Cassandra, Riak |
...
Aurora 讀 writer endpoint 是 CP 行為（保證一致），讀 reader endpoint 是 AP 行為（可能拿到舊值但不會停）。同一個 Aurora 叢集，兩種行為都有，看 query 走哪個 endpoint。
...
大部分系統從 Leader-Follower 開始。等到某次不一致真的造成損失，才值得換成 consensus；寫入量大到單一 leader 處理不來，才輪到 leaderless。
