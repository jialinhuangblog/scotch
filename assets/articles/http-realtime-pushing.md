...
subtitle: "從 HTTP/1.1 的六條連線，到 WebSocket 借 HTTP 握手升級協議。每一步都在繞開 server 不能主動送資料這個限制。"
chapter: "realtime"
...
三十多年來，工程師為了繞過這個限制做出一整套技術，每多一層都是因為前一層有缺。
    ...
    → Long Polling 問了你不要急著回（hold connection）
      → SSE 一條 GET 永遠不關（單向 stream）
        → WebSocket 借 HTTP 握手後升級成雙向 TCP
          → Socket.IO 當相容性 fallback（先 long-poll 再升級）
```
...
每條連線**一次只傳一個資源**。資源 A 傳完才能傳 B，這叫 **head-of-line blocking**（per connection）。50 個資源同時要載入，前 6 個先傳，後 44 個排隊。
...
跨分頁也會共用：兩個分頁連到同個 origin，瀏覽器把它們合併到同一條 TCP 上。Connection coalescing 則是跨 hostname 合併。不同 hostname 如果都在證書的 SAN 裡、DNS 又解到同一個 IP，瀏覽器也會把它們放進同一條 TCP（RFC 7540 §9.1.1）。CDN 部署時會刻意讓證書跟 DNS 滿足這個條件。
...
為什麼建在 UDP 上不在 TCP 旁？兩個原因：TCP 在 OS kernel 裡，很難修改；中間 box（NAT、防火牆）只認 TCP / UDP，新 protocol 的封包會被它們丟掉。UDP 幾乎只負責把 packet 送出去，其他功能都留給上層自己做。kernel 為何難改、TCP 跟 UDP 的 pseudo code 對照，見 [HTTP/3 over QUIC](chunk://http3-quic)。
...
QUIC 另外用 connection ID 認連線，IP 換了照常用，所以**手機從 Wi-Fi 切到 4G 時連線不必重建**。

---

## 應用層：Polling / SSE / WebSocket 在解什麼

傳輸層演進處理「怎麼讓 N 個 request 共用一條 TCP」。但 server 主動推送，HTTP 本身就沒有設計。
...
1000 個 user 每 5 秒問一次，就是每秒 200 個 request，而且大多是空回應。延遲也差，最壞要等一整輪才看到新資料。量小可以用，規模一大就不行。

### Long Polling：server 故意不回

Client 發一個 request，**server hold 住**，等有新資料或 timeout 才回。server 端的 code 長這樣：
...
Server 要記住哪些 request 正在等待。10 萬個 user 同時 hold，就是 server 上開著 10 萬條 TCP，要用 non-blocking I/O 加 pub/sub 才能處理。詳見 [long-polling chunk](chunk://long-polling)。

最大的優點是**它就是 HTTP**。任何 firewall、proxy、HTTP/1.0 環境都能用。Socket.IO 的 fallback 就是 long polling，先確保能連上，可以升級才升級。
...
`Content-Type: text/event-stream` 是 SSE 專用格式。在 HTTP/1.1 下靠 chunked transfer encoding，server 不用事先知道整個 response 多大；HTTP/2 本身就是分 frame 送，不需要 chunked。瀏覽器用 `EventSource` API 接收：
...
HTTP/1.1 下 SSE 跟 WebSocket 都會佔掉 6 條 pool 中的一條。多開幾個分頁、每頁再開一條通知用的 SSE，pool 很快就用完，正常的 API request 只能排隊。
...
[gRPC](chunk://grpc-http2) 強制綁 HTTP/2，因為它需要下面這些功能，HTTP/1.1 都做不到：
...
| 通知量暴增 | Kafka [fan-out](chunk://fan-out) + worker 寫 per-user queue |

**離線 / 關閉時怎麼收：** 頁面開著就用自己的 SSE/WebSocket 直接推；頁面關了，走 **Web Push + Service Worker**：

1. Server 把通知 POST 給**瀏覽器廠商的 Push Service**（Chrome 走 Google 的 Firebase Cloud Messaging、Firefox 走 Mozilla 的 autopush、Safari 走 Apple Push Notification service），不是連瀏覽器。拿到 HTTP 201 就代表 Push Service 收下了，server 之後掛掉也不影響送達。
2. 那條「裝置 ↔ Push Service」長連線由**瀏覽器 / 作業系統**維護（一個瀏覽器共用一條，不是每個網站各一條）。裝置在線就即時送下去；離線時訊息**存在 Push Service 的雲端**（Google / Mozilla / Apple 的伺服器，而且是它也解不開的加密 blob），**不在裝置上**。裝置離線期間根本還沒收到它，所以才沒跳通知，要等重連 Push Service 才補送。訊息放超過存活時間（Time To Live，Firebase Cloud Messaging 預設約四週）就直接丟掉。
...
| 相容性 fallback | [Long polling](chunk://long-polling) fallback（Socket.IO 自動降級） |
...
一家中型 SaaS 要新增「即時通知中心」：user 在系統內被 @ 提到、被指派任務、收到評論時，網頁 icon 要立刻出現紅點。需求盤點下來是這樣：

- **單向**：server 推，client 不需要回傳任何資料
...
上線半年，產品要加「協作白板」，讓多個 user 同時編輯。這時才升級成 WebSocket，獨立部署 gateway。**通知中心不動，繼續走 SSE**。

### 什麼時候會後悔

SSE 適合 server→client 單向。要是哪天通知要加上「按鈕快速回覆」（例如直接在通知裡回評論），就會出現雙向 traffic。這時有兩條路：
1. 通知還是 SSE，回覆走另一個 REST endpoint（最簡單）
2. 全面換 WebSocket（投資 gateway 基礎建設）

要是 80% 的通知不需要互動，就繼續用 SSE 加額外的 REST。互動比例高了，才值得再投資一筆 infra 全面換成 WebSocket。
...
WebSocket 升級協議，雙向（但要管 stateful）
  ↓
Socket.IO 當 fallback（先 long-poll 再升級）
```
