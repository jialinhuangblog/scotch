...
slug: why-kafka
date: 2026-07-12
subtitle: "Kafka 只有一個答案是別人給不了的：它把訊息留著。replay、多個下游、CDC、event sourcing 都從這點來，代價是 retention、順序、offset 都要自己管。"
chapter: "messaging"
...
Kafka 在四個問題上的位置（[上一篇](article://message-system-axes)講的那四題）：送法看 consumer group、推拉是 pull、位置在後端。這三題 RabbitMQ、SQS 多少也答得出來。真正只有 Kafka 給的，是存法那一題的答案：**留著**。訊息讀過不刪，consumer 自己記讀到哪一筆。

Kafka 那些聽起來各自獨立的用法，replay、多下游、CDC、event sourcing，說到底都是「留」帶出來的。

---

## 留著，所以壞掉可以重來

consumer 掛掉的情況 queue 其實能處理，因為沒 ack 的訊息會重新投遞。分歧發生在 ack 之後。queue 刪掉，Kafka 留著，consumer 記的只是一個叫 offset 的書籤，標著讀到第幾筆。
...
補歷史也是用重讀 log 的方式。一個全新的服務今天上線，想要過去三個月的資料，從 offset 0 讀到尾就有了。offset 怎麼運作、多個 consumer 怎麼分讀，看 [The Log](article://the-log) 和 [consumer groups](chunk://consumer-groups)。
...
三個速度完全不同的下游讀同一份資料，互不影響。

這跟 SNS 那種 fan-out 不一樣。SNS 是發布的當下就把訊息複製成 N 份，推給 N 個訂閱者；Kafka 只存一份，讀的人各自來拿。前者的成本在寫入端，後者把選擇權留給讀取端。訊息留著，所以讀取端可以自己決定什麼時候讀、讀幾次、幾個人讀。

---

## 留著，所以 DB 改了能通知出去

假設訂單狀態存在 Postgres，付款、庫存、通知三個服務都要在它變動時反應。最直覺的做法是 app 寫完 DB 再自己發事件，也就是 dual-write，但這是兩次獨立的寫入。要是 DB 寫成功、事件發失敗，DB 裡有這筆變更，下游卻永遠收不到。
...
**CDC 搭的是「DB 本來就要寫這一筆」的便車。**訂單那一列是系統的真相，非寫不可，CDC 只是把這個既有的寫入撈進 Kafka。反過來，如果一件事根本不需要在 DB 留一列（例如一次點擊），就別為了用 CDC 去硬造一筆 DB 寫入，app 直接發進 Kafka 就好，不用繞一趟 DB。
...
為什麼有人要把最慢的 DB 從主角換成下游？上面那三行事件就是理由：任何時間點的狀態都能重放出來，要一份新視圖（換個資料庫、換個聚合方式）也只要重放一次。代價是查當下狀態多一道手續，得先物化成視圖才查得到；而且視圖通常慢一拍更新，剛寫進 log 的那筆，馬上查不一定查得到（read-after-write 不再保證）。

需要關聯查詢、外鍵、約束當真相，就是 DB-first 配 CDC；事件流本身就是真相，就是 log-first 走 event sourcing。不管哪種，底下都是 Kafka 把 log 留著，差別只在 DB 和 log 誰衍生誰。

---

## 留著，就得自己管這些

訊息留著，才有上面這些用法，但下面這些要自己處理：
...
RabbitMQ、SQS 把這些都省了（ack 完就刪，沒有 retention 要管、沒有 offset 要記），換來的是它們也給不了 replay。要 replay，就得自己管上面這些。
