---
title: "UPDATE 一筆 row，InnoDB 跟 Cassandra 各自做了什麼"
...
# UPDATE 一筆 row，InnoDB 跟 Cassandra 各自做了什麼

`UPDATE users SET name='Carol' WHERE id=1`

這筆操作在 InnoDB（B+ Tree engine）和 Cassandra（[LSM Tree](chunk://lsm-tree) engine）裡，從 Client 送出 query 到資料寫進磁碟，走的路完全不同。
...
大部分 OLTP 場景裡，常用資料的 page 幾乎都在 Buffer Pool 裡，命中率通常超過 99%。
...
redo log 只記改完之後的值。舊值 'Bob' 放在 undo log，下一步會講。
...
不需要找 page、不需要找位置。直接插入記憶體結構是 O(log N) 的操作，寫完就回覆 Client OK。

這條路不經過 Buffer Pool，也沒有 dirty page 要回寫。

### Step 3：沒有 MVCC

Cassandra 不需要 MVCC，因為它沒有跨多筆資料的 transaction，也沒有 isolation level，只有針對單一 partition 的 lightweight transaction。每筆寫入都帶 timestamp，讀的時候取 timestamp 最大的那個就是最新值。
...
Memtable 到了設定的大小上限，就整包排好序寫到磁碟，變成一個 SSTable 檔案。Sequential write，不需要找位置。
...
InnoDB 最後維護的是一棵排好序的 B+ Tree，Cassandra 則不斷產生新的 SSTable，再定期合併。讀取效能跟寫入效能的取捨，就是從這個差別來的。
...
這是儲存引擎的 RUM 取捨：讀（Read）、寫（Update）、空間（Memory）三種成本，最多壓低兩種，剩下那種就會變差。B-tree 讀快，代價是寫放大，也就是一筆 update 實際寫進磁碟的量是原資料的好幾倍。LSM 寫快，配上壓縮之後空間通常也省。它的主要代價在讀，因為要掃描多個 SSTable。RUM 跟 CAP 一樣不能全拿，但 RUM 是連續、可以調整的取捨。同一個 LSM 換個 compaction 策略，就能在讀、寫、空間之間重新分配成本，不是 CAP 那種硬性二選一。真要三種都壓低，就得多投入 RAM 跟硬體，也就是花錢，或換更好的演算法讓三種成本一起下降。
