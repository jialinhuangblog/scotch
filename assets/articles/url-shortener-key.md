...
subtitle: "Hash 截斷後會撞，counter 可以被遍歷，Snowflake 的 key 太長。實務上最常用 counter，再加一層 bit shuffling 或加密，讓外部看到的 key 猜不出下一個。"
...
給一個長 URL，回傳一個短 URL，點短 URL 能 redirect 回原始的長 URL。整題真正要解的，是長 URL 怎麼變成一個 7 個字元的 short key。
...
即使是 Bitly 等級的量，7 個字元也夠用幾百年。

6 個字元呢？62^6 ≈ 568 億，每天 2000 萬只夠用 7.8 年，餘量不夠，所以選 7。

---

## 方法一：對 URL 做 Hash

最直覺的做法是把 long URL 丟進 hash function，取前 7 個字元。

```text
MD5("https://example.com/very/long/path")
= "bab0087156708cf874ea253a83b8fd5c"
...
MD5 輸出 128-bit，取 7 個 hex 字元只有 28 bit（2.68 億種）。下面會算到，存到一萬多筆撞到的機率就超過一半。用 SHA-256 也一樣，因為最後都要截斷。

如果改用 base62 取 7 個字元（3.5 兆種），collision 延後很多，但依然存在。原因是 birthday paradox：不需要用完整個空間才撞，碰撞發生的時間遠比直覺早。
...
### 同一個 URL 永遠算出同一個 hash

同一個 long URL 永遠算出同一個 hash，這叫天然 dedup（natural deduplication）。不需要額外查 DB 確認「這個 URL 縮過沒」，因為同一個輸入永遠得到同一個輸出。
...
counter = 3,521,614,606,207 → base62 → "ZZZZZZZ" (7 位的上限)
```

這裡的字元順序是 0-9、a-z、A-Z，所以 0 最小、Z 最大。
...
| 洩漏規模 | `/users/58000` → 大約 5.8 萬用戶 | 推不出總量 |
...
→ base62 → "28Y1bpg4Wl2" (11 字元)
```

每台機器自己產 ID 也不會撞，而且 ID 自帶時間排序。

### 問題：key 太長

幾個 64-bit 整數轉成 base62：

```text
7183294852610          → base62 → "22sT39qW"     （8 字元）
9999999999999999       → base62 → "JNBBGsgR9"    （9 字元）
最大 64-bit：2^63 - 1  → base62 → "aZl8N0y58M7"  （11 字元）
```

Base62 跟十進位一樣，數字越大字元越多。Counter 從 1 開始慢慢長，前期 key 很短。Snowflake 不一樣，timestamp 放在最高位，而且是從 epoch 開始算的毫秒數。以 Twitter 的 epoch（2010 年）來說，timestamp 左移 22 bits 之後，現在的 ID 已經是 1.8×10¹⁸ 這個量級，所以一開始就是 11 個字元。

硬截成 7 個字元，又回到 collision 問題。要塞進 7 個字元也很難，因為 7 個 base62 字元只有大約 41.7 bits，光是 Snowflake 的 timestamp 就佔 41 bits。得大幅縮短 timestamp、machine ID、sequence 的位數，可用年限跟機器數都會跟著變小。
...
301 和 302 都是 redirect，瀏覽器收到之後的處理不一樣：
...
>
> 唯一回傳同一個 key 的場景是純公共工具型（沒有帳號、沒有 analytics），可以省空間，但現在幾乎不存在了。

---

> **「Counter 的 key 可以被遍歷。怎麼防？」**
>
> Counter 產出的 key 是連續的：`4c91`、`4c92`、`4c93`。知道一個就能猜出前後所有的。解法是 counter 產出的數字只當內部 ID，不直接當 short key，中間加一層轉換，讓外部看到的 key 不可預測。
>
> ```text
...
> ```
>
> 常見的轉換有兩種：bit shuffling 跟加密。兩種都是 bijective function（雙射函數），每個輸入對應唯一一個輸出，每個輸出也只對應一個輸入。不會有兩個 ID 轉成同一個 key（不碰撞），也可以從 key 反推回 ID（可逆）。
>
> ```text
...
> ```
>
> 攻擊者看到 `Rn3vL8w` 和 `Xk9mP2q`，沒有 secret key 就推不出下一個是什麼，也不知道這兩個是連續 ID。跟 bit shuffling 的差別在於洗牌規則被猜到就能破解，AES 的 128-bit key 則沒辦法暴力破解。
>
> 上面的數字是示意。AES 的 block 固定是 128 bits，直接加密一個 counter 會得到 128-bit 的輸出，轉成 base62 大約 22 個字元，不是 7 個。要讓輸出留在 7 個字元以內，實務上用 format-preserving encryption（FPE，例如 NIST 的 FF1），輸出跟輸入落在同一個數字範圍。
...
> **「hash 方法需要每次都查 DB 確認不碰撞。一秒一萬個建立請求，DB 能處理嗎？」**
>
> 在 DB 前面放一個 Bloom filter（詳見 [bloom-filter](chunk://bloom-filter) chunk）。先問 Bloom filter，「一定不存在」就直接用，不查 DB。大部分新 key 都不會撞，只有極少數 false positive 才真的打到 DB。10,000 QPS 可能只有幾十個落到 DB。
>
> Bloom filter 存在 RAM 裡，查一次幾十奈秒。DB 查詢要走 SSD + 網路，慢 10,000 倍以上。系統啟動時從 DB 載入所有已存在的 key 到 Bloom filter，之後每次新增也同步加入。用 RedisBloom 的話可以跟著 Redis 一起持久化，重啟不用重建。
...
>
> 如果被追問「已經上線了才加 custom alias 功能」：舊 key 不動，新的 custom alias 撞到舊 key 就拒絕，未來系統產的 key 改成不會跟 custom 格式撞的格式。

---

> **「如果同一個 long URL 用同一個 idempotency key 送了兩次，第二次應該回傳什麼？」**
>
> 第二次送進來的時候，第一次可能已經處理完，也可能還在處理（詳見 [idempotency](chunk://idempotency) chunk）：
>
> ```text
...
>   → 不用 429（429 是 rate limiter 拒絕太頻繁的請求，語義不同）
> ```
...
> -- 回傳更新後的 40001，所以這次領到的 range 是 [30001, 40000]
> ```
>
...
>
> 回頭問「為什麼需要 dedup」。如果用 counter 方案，每個號碼唯一，天生不重複，根本不需要 dedup 機制。
...
這些 key-value pair（short key → long URL）量大之後要存在哪裡，是[下一篇](article://url-shortener-store)的題目。
