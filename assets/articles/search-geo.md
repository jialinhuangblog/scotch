...
slug: search-geo
subtitle: "文字比對、地理範圍、facets 很難放進同一個 Postgres query。Elasticsearch、geohash、CDC 怎麼分工。"
chapter: "search"
...
Airbnb 的 search 介面是左邊一排 listing，右邊一張地圖，地圖一拖就重新查詢。底下要同時做文字 match、地理 filter、多重條件 filter、按 relevance + rating 排序，還要順便算 facets（每個 amenity 有幾筆）。

Airbnb 規模約 700–800 萬個 listing（[2023 約 700 萬、2024 約 800 萬](https://www.demandsage.com/airbnb-statistics/)）。MAU 跟每秒查詢量官方沒公布，這裡用「上億用戶、每秒數千查詢」的量級來設計。

這些要求很難全塞進 Postgres 的一個 query，就算寫得出來也慢。`ILIKE` 在百萬筆上要逐筆掃描，而 FTS 加 PostGIS 加 GIN index 再加 GROUP BY 算 facets，optimizer 不見得選對 plan。所以 search workload 拆給 [Elasticsearch](chunk://elasticsearch)、Postgres、[CDC](chunk://cdc) 分工。
...
地圖一拖就要立刻重查，而這六個維度各需要不同的 index：

- 文字 search → 要 [inverted index](chunk://search-inverted-index)
- 地理範圍 → 要 [geohash](chunk://geohash) / R-tree（用矩形包住一群點、一層一層往下分的空間索引）
...
Postgres 是 source of truth，ES 是衍生 view。一致性靠 [CDC](chunk://cdc) 維持。
...
ES 的 `geo_point` 欄位內部用 BKD tree 索引（很早期的版本才用 [geohash](chunk://geohash) 前綴），三種 mode 都能跑。Postgres 要 PostGIS 才有等價能力。
...
**B. Two-phase search**
ES 取出 candidate（忽略 availability），帶著 ID 列表去 Redis / availability service 批次查詢可用性，過濾後回傳。**多一個 round trip 但資料新鮮**。

這裡的設計用 (A) 加 UX fallback 當預設，需要更高新鮮度才上 (B)。

---

## Ranking：兩段式

只用距離跟價格排序，沒考慮評價、房源品質跟使用者偏好。實際的 ranking 分兩段：
...
- **`facets` 跟 search 一起回**，好讓 count 是 0 的選項先灰掉，不會被選了才發現沒結果
- **`cursor` 而不是 `offset`**：用 offset 翻到很後面時，ES 每個 shard 都要取出 from + size 筆再合併，預設超過 10,000 筆就直接拒絕。cursor（`search_after`）只要從上一頁最後一筆接著取
...
一間旅館訂房新創，MVP 只有 5,000 listing，全部靠 Postgres + PostGIS 跑。月 GMV 開始破百萬，listing 衝到 80,000，產品要加 amenity filter + autocomplete + map cluster。

第一個壓力點：amenity filter。listing.amenities 是 array，Postgres 用 GIN index 查 `array @> ['wifi','kitchen']`，加上 price range、bbox、city_id 四五個 filter 一起，query plan 開始選錯 index，出現 10 秒以上的 query。

第二個壓力點：facets。每次 search 都要 GROUP BY amenity 算 count。80,000 筆還行，但預期年底破 50 萬筆，GROUP BY 要處理的筆數是現在的六倍多。
...
ES 是衍生 view，consistency 是 eventual。某天客服收到投訴：「我下架了 listing，搜尋結果還有它」。Sync delay 在 ES 重啟、Kafka lag、consumer 卡住時可能拉到分鐘級，這時要補上這些處理：

1. ES delete 改成 soft delete + 標記 `hidden`，搜尋濾掉這個 flag
2. 監控 Kafka consumer lag，超過 30 秒 alert
3. Detail 頁進去再驗一次（雙重檢查）

要是業務變成「listing 下架要立刻消失，不能容忍 5 秒 delay」，就要把 search 流量導回 Postgres 的某條 path（例如 city_id + 簡單 filter），ES 只跑複雜 query。

---

## 來源

- Airbnb active listings：2023 約 700 萬、2024 約 800 萬，[demandsage Airbnb Statistics](https://www.demandsage.com/airbnb-statistics/)（彙整 Airbnb 官方財報數字）。MAU 與每秒查詢量 Airbnb 未公開，文中相關量級是假設的。
