...
[Cache](article://cache-hot-key) 處理讀取、[queue](article://queue-peak-shaving) 處理寫入，兩個都是替 DB 吸收流量。它們處理的是流量大小，rate limiter 看的則是哪個 client 送太多。

一個 user 每秒送一萬次，其他人全卡住。cache 再大、queue 再深都沒用，資源被一個人用完，系統對其他人就等於當機。Rate limiter 判斷的是**這個 client 的 request 該不該放行**。
...
同樣 10,000 req/s，意義可以完全不同。

| 情境 | Key | 該限制嗎 |
|---|---|---|
| 單一 user 送 10,000/s | user_id | 限制 |
| 1,000 個 user 各送 10/s | user_id | 不限制（每人都正常）|
| 匿名流量 | IP | 依情境 |
| 第三方呼叫 API | API key | 限制（保護 quota）|

Key 決定「誰跟誰分開算」。key 選錯的話，該限制的 client 根本限制不到。

企業環境用 IP 當 key 看似合理，但整棟大樓可能共用一個 NAT IP，限制一個 IP 就等於限制了兩千人。這比較是業務面的判斷，跟用哪套演算法關係不大。
...
想像一個桶。每秒往桶裡丟固定數量的 token，容量有上限。request 進來拿一個 token，沒有 token 就拒絕。
...
每次 request 算 elapsed time，補 token，扣一個。但讀、算、寫這三步不是原子的。
...
解法是把整段邏輯寫成一段 script 送進 Redis，讓 Redis 在 server 端一口氣跑完，中間不會被其他 request 插隊。Redis 用 Lua 語言寫這種 server-side script：
...
用 `EVAL` 送出這段 script，Redis 會在 single-threaded event loop 裡一次跑完，讀、算、寫之間不會有別的 request 插進來。
...
Redis 用 Sorted Set，score 放 timestamp，Sorted Set 本身就按 score 排序，也就是按時間排序。跟 Token Bucket 一樣，清過期、數數量、加新的，這三步要綁成一個原子操作，否則兩個 request 同時數到 count=9 都放行。一樣包成 Lua：
...
把 counter 放在本地記憶體，一台機器算得準。可是一旦前面加了 load balancer、請求被分到好幾台，每台都「只」看到自己那份，總數就會少算。
...
Redis 單機簡單操作大約十萬次量級（官方 benchmark 沒開 pipeline 約 18 萬，這裡抓 10 萬當保守估）。Rate limit 每次檢查要 3-4 個 op（讀、算、寫），所以實際能處理的檢查次數，大約是 op 數再除以 3 到 4。
...
Per-user key（`rate_limit:user_123`）本來就分散，不同 user_id 是不同 key，consistent hashing 自動分到不同 node。Hot key 問題出在**全域 endpoint 的 rate limit**（`rate_limit:/search`）。

所有人打 `/search`，rate limiter 都對同一個 Redis key 做 INCR。Consistent hashing 把這個 key 分配到某一台 node。不管 cluster 有 4 台還是 400 台，**一個 key 永遠只落在一台**。那台滿載，其他台閒著。
...
拆完的 10 個 key 名字不同，consistent hashing 會把它們分散到不同 node。
...
[Rate limiter](chunk://rate-limiter) 在最前面限制每個 client 的請求量，[Bloom filter](chunk://bloom-filter) 在 DB 前面過濾掉不存在的 key。兩個處理的問題不同，所以不能互換。

---

## 跟前兩篇的對照

| | Cache（讀取） | Queue（寫入） | Rate Limiter（單一 client） |
|---|---|---|---|
| 保護什麼 | DB 不重複讀 | DB 不用一次處理寫入尖峰 | 所有資源不被單一 client 耗盡 |
| 失效時 | 雪崩 / 穿透 / 擊穿 | 訊息遺失 / 重複消費 | 單一濫用者用光所有資源 |
...
一個 SaaS 平台把 API 開放給第三方，免費方案跟付費方案共用同一組 server。某天凌晨兩點，免費方案的某個用戶寫了腳本不停呼叫 API，付費客戶的 response time 從 50ms 飆到 3 秒。
...
Token bucket 允許 burst，對 API 場景幾乎都適合。但要是下游有嚴格合約（例如第三方 API 按次計費，超過要付錢），burst 會讓用量偶爾超額。這時要換 sliding window，或至少在計費路徑上加一層精確計數。
