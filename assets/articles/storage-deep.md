...
每一層是一個 node，對應磁碟上的一個 page。從 root 往下走，每往下一層就排除掉大部分不可能的範圍。100 萬筆只要 3-4 層就能定位。O(log n)。
...
Clustered index 不是 MySQL 獨有，但各家行為不同。InnoDB 的 PK 強制是 clustered index，沒得選。SQL Server 的 PK 預設是 clustered 但可以改成其他欄位。PostgreSQL 沒有 clustered index，資料存在 heap 裡，所有 index（包括 PK）都是 secondary，查詢完 index 都要回 heap 取資料。
...
`WHERE email = 'b@x.com'` 先走 email index 找到 PK=2，再用 PK=2 去 clustered index 查詢一次，取得整筆資料。兩次 B-Tree lookup。這叫 bookmark lookup。PostgreSQL 的話，所有查詢都要回 heap，等於每次都是 bookmark lookup。
...
`WHERE status = 'pending'` 直接讀那個 bit array，不用掃任何 row。多條件組合用 bitwise 運算：`WHERE status = 'pending' AND region = 'asia'` 就是兩個 bitmap 做 AND。適合值很少的欄位（enum 型別）。值太多的話每個值都要一整條 bitmap，佔用的空間會太大。
...
每多一個 index，每次 INSERT 和 UPDATE 都要多寫一份 index 結構。五個 index 就是六次寫入（一次 data + 五次 index）。

---

## Storage Engine：為什麼 InnoDB 贏了

MySQL 的 storage engine 是可插拔的，同一台 MySQL 裡，不同 table 可以用不同 engine，其他 DB 基本上沒有這個選項。這個彈性在早期帶來一個選擇題：
...
MyISAM 輸得這麼徹底，為什麼還當了十五年預設呢？

原因在 1995 年的市場。MySQL 的目標用戶是 PHP 網站、留言板、CMS：讀多寫少，掛了重跑就好，要快要簡單。這群用戶不需要 transaction。MyISAM 不是做不出 transaction，是刻意省掉，換更快的讀和更簡單的部署。transaction 這塊 MySQL 用插拔介面留給別人做：InnoDB 就是芬蘭公司 Innobase 做的第三方 engine，2001 年才併進 MySQL 3.23。後來 web 應用開始收錢，電商、金流出現，「掛了重跑就好」不再成立。MySQL 5.5（2010）把預設 engine 改成 InnoDB，現在幾乎沒有理由再用 MyISAM。

Oracle 和 SQL Server 從第一天就走另一條路。他們賣的是銀行、ERP、訂單系統，客戶的底線是錢不能算錯、機器掛了資料不能壞。所以整個 DB 從一開始就圍繞 transaction、row lock、crash recovery 設計，engine 跟 DB 是同一塊，拆不開，也不需要拆。PostgreSQL 出身學術界，transaction 同樣內建在核心。

InnoDB 贏的理由可以拆成四個特性：MVCC、ACID、row lock、crash recovery。商用 OLTP 的客戶第一天就需要這四樣，所以其他家一開始就內建，也沒有「選 engine」這個問題。MySQL 起家的市場不需要它們，事後才靠 InnoDB 補齊：
...
嚴格說 PostgreSQL 12 之後有 table access method 介面可以換 engine（預設 heap），SQL Server 也有 In-Memory OLTP 這顆獨立 engine。但實務上幾乎沒人換，所以說 engine 跟 DB 拆不開，大致成立。

InnoDB 補齊之後，還有幾處做得比別家好。clustered index 讓 PK 查詢少一跳，PostgreSQL 沒這個選項。舊版本集中放在 undo log，用完就清，不像 PostgreSQL 把死掉的版本留在 heap 原地，等 vacuum（PostgreSQL 清掉死版本的背景程序）事後回收。所以功能追平了，差別剩在 clustered index 跟舊版本放在哪裡。

---

## ACID：選了 InnoDB，換到的是什麼

「支援 transaction」這句話本身不保證什麼。能寫 BEGIN + COMMIT，不代表出事時資料不會壞。選 InnoDB 而不是 MyISAM，換到的就是 [ACID](chunk://acid) 四個保證：
...
這四個字母底下實際怎麼跑（undo log、version chain、redo log 重放、WAL 跟 MVCC 怎麼分工），是另一篇的主題：[UPDATE 一筆 row，InnoDB 跟 Cassandra 各自做了什麼](article://storage-internal)。
...
`WHERE sale_date = '2024-02-15'` 時，DB 知道只要掃描 sales_2024，不碰 sales_2023。這叫 partition pruning。5 億筆切成 5 份，每份 1 億，掃描量直接降 80%。
...
Partition 在同一台 DB 裡。磁碟空間、CPU、記憶體都是那一台的上限。5 億筆切成 5 個 partition，每個 partition 還是 1 億筆。如果一台機器連 1 億筆都處理不了呢？
...
Sharding 最直接的代價是 application 要自己決定每筆 query 去哪台 DB。這段 routing logic 會出現在 codebase 裡每個查詢資料的地方：
...
沒有 sharding 的時候，`getUser` 就是一行 SQL。有了 sharding，每個 query 都要先算 shard，跨 shard 的要自己拆開、自己合併，每個 data access layer 都得寫這段邏輯。
...
**跨 shard transaction。** 扣 user A 的錢（Shard 0）加到 user B 的帳戶（Shard 1）。一個 transaction 跨兩台機器，需要分散式 transaction，例如 2PC（two-phase commit，兩階段提交）：先問每台能不能 commit，全部回答可以才真的 commit。複雜度會大幅增加。
...
三個月前 1 萬筆，現在 800 萬筆，查詢從 2ms 變成 1.2 秒。底下是一條示意的時間軸。數字都是舉例，觸發訊號才是判斷依據。

第一步的訊號是查詢慢到超過 latency 目標。在 `user_id` 和 `created_at` 上加 composite index，掃描變成索引查找，查詢回到個位數 ms。

六個月後資料來到幾千萬筆。這時會想動 partition，通常是因為**單表開始難維護**：加欄位要鎖很久、重建 index 跑好幾小時、vacuum / analyze 時間拉長，hot 跟 cold 的資料混在一起讓 cache 命中變差。按 `created_at` 做 range partition、每月一個之後，大部分查詢只碰最近幾個月，舊資料也可以順手歸檔。

兩年後資料上看數十億筆。但真正需要 sharding 的不是 row 數（一台 DB 裝幾十億筆很常見），而是**單台的寫入吞吐或 IO 達到上限**：每秒寫入接近磁碟頻寬、CPU 長時間滿載、連線數到上限。按 `user_id` hash sharding 分到多台，每台只處理一部分 workload。代價是 application 要加 shard router，跨 shard 的報表查詢改成非同步 batch job。

---

## 寫入遠多於讀取，或機器會掛的時候

**[LSM-Tree](chunk://lsm-tree)** 是 B-Tree 以外的另一種選擇。B-Tree 讀快寫慢，LSM-Tree 寫快讀慢。Cassandra、RocksDB、LevelDB 都用 LSM-Tree。適合寫入量遠大於讀取量的場景。
