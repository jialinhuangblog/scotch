...
date: 2026-06-14
subtitle: "一個跑在 production 的訂單後端，照做了 Repository、Domain Service、Application Service 跟 Anti-Corruption Layer。Aggregate Root 只用在有 invariant 要守的 Order 上，Bounded Context 只有一個，因為沒有會分裂的 subdomain。"
chapter: "extras"
...
讀完 Eric Evans 那本 [DDD](chunk://ddd) 藍皮書，裡面有 Ubiquitous Language、[Bounded Context](chunk://bounded-context)、[Aggregate Root](chunk://aggregate)、[Repository](chunk://repository-pattern)、[Domain Service](chunk://domain-service)、[Application Service](chunk://application-service)、[Domain Event](chunk://domain-event)、[Anti-Corruption Layer](chunk://anti-corruption-layer)。八個概念，每個都對得到以前 code 裡碰過的問題。

第二天打開一個跑在 production 的訂單後端，準備照著抄。grep `AggregateRoot`、grep `class.*Aggregate`，零個結果。entity 跟 entity 各自獨立，沒有所謂 root。
...
TypeScript 是**結構化型別（structural typing）**，method 名字跟簽名對得上就算實作，**技術上不寫 `implements` 也相容**（那時 infra 連 domain 介面都不用 import，解耦最徹底）。不過實務上多半還是會寫 `implements`（NestJS 尤其這樣），為了讀起來清楚、也讓 type checker 在 class 的簽名跟介面不一致時早點報錯。

結構化型別不是 Go 的專利，TS 也是。兩者都在編譯期檢查，這點跟 Python / Ruby 那種執行期的 duck typing 不同。**「一定要寫 implements」是 Java / C# 那種 nominal 型別的規則。**

這條大概是抄得最完整的一條。domain 定義介面，infrastructure 照著實作，application 只認介面、不管底下是哪家 DB。
...
這個 class 在編譯期就依賴 `PrismaOrderRepository`，而且一定得 import Prisma。要換 Mongo、要在測試裡塞假的，只有進這個檔改 source 一條路。
...
`new` 搬去哪其實不重要。這個 class 不再自己決定用哪個 repository，改由外面傳進來。同一個沒改過的 class，prod 跟測試可以傳進不同的實作：
...
它依賴的是 `OrderRepository` 介面、碰不到具體 class，所以測試可以傳假的進去，換 DB 也不用修改它。
...
return type 宣告的是 domain 的 `OrderRepository`，回傳的實體卻是 `PrismaOrderRepository`。其他檔案要嘛只待在 domain、要嘛只待在 infrastructure，只有 `di.ts` 跨兩邊。`new` 只出現在這裡，換 DB 那天也只改這一行：

```typescript
return new MongoOrderRepository(mongo);   // domain 跟 application 一行不動
```

平常寫 NestJS 其實很少自己 `new`。手刻 `di.ts` 是為了把機制攤開看，實務上 NestJS 的 DI container 會負責執行這串 `new`，只要在 module 宣告綁定：
...
`OrderRepository` 是 TS interface，runtime 不存在，不能直接當 token。所以實務上會把它寫成 `abstract class`，或用 string/symbol token 配 `@Inject()`，這也是 NestJS 圈常寫 `abstract class` 的原因。

**DI 把 `new` 集中到一個地方，讓其他 code 依賴介面、不依賴實體。**

如果這個 repository 永遠只有一種實作，又不寫測試，DI 多出的這一層就沒有用處，因為沒有第二種實作要換進來，也沒有測試要傳假的進去。

## rule 3: Anti-Corruption Layer（外部物流商的翻譯層）

訂單要接外部物流商查貨態。FedEx API 回的是它自己的格式（欄位名是縮寫，狀態是代碼，日期是 `20260620` 這種字串）。這些 type 一旦傳進 application，application 就得認得 FedEx 的格式，哪天換 DHL，所有讀這些欄位的 code 都要改。

ACL 的解法是寫一個 translator，把 FedEx 的格式翻成 domain 的 `Shipment`：
...
**把它簡單想成 adapter 就好**，design pattern 那個 adapter。ACL 常常就是一堆這種 translator 組成的。一般 adapter 只要把兩個介面接起來，ACL 還要讓外部的欄位名跟概念進不了 domain。

## rule 4: Domain Service 跟 Application Service 分清楚

書上分兩種 service：[Domain Service](chunk://domain-service) 是純業務規則、零 I/O；[Application Service](chunk://application-service) 是編排多個步驟、有 I/O。
...
**運費怎麼算、訂單成不成立這種業務判斷在 domain；把這些步驟接起來、加上讀寫資料庫，在 application**。`PlaceOrderService` 注入的依賴**全是 domain 介面**，沒一個是具體的 Prisma / FedEx class，所以它永遠不知道下面是哪家 DB、哪家物流。
...
`Customer + Address` 沒有這種規則。一個客戶要存幾個地址都行、改一個地址也不影響別的，兩邊沒有綁在一起的規則。硬把 `Customer` 當 root、所有 address 變更都得經過它，只是讓 code 變複雜，沒有多守住任何規則。所以 address 就自己一個 repository，跟 customer 各自獨立。

書上會說後者「失去 ownership invariant」，但本來就沒有 invariant 可失去。這跟 [Vaughn Vernon 的 Effective Aggregate Design](https://www.dddcommunity.org/library/vernon_2011/) 一致：看的是真的有沒有要一起守的規則，不是看物件之間關聯多深、或包起來方不方便。
...
所以讀 entity 那塊 code，詞彙跟業務的講法一致；讀到付款就得切換成 Stripe 的那套講法。這是刻意選的 trade-off：如果金流也要做 UL，就得包一層自己的 abstraction，把 Stripe 的詞彙全部換掉，維護負擔加倍。團隊選擇讓 Stripe 的詞直接出現，但用 ACL 限制它只出現在 infrastructure 層，不進 domain。
...
這個訂單後端整個是**一個 bounded context**。`Order`、`Customer`、`Cart` 在這裡只有一種意義，沒有跨 subdomain。這個不做也合理，因為規模還沒大到需要拆成多個 bounded context。書上本來也沒強迫：

> when context boundaries cause friction, then introduce explicit contexts

## 對照表：8 條 vs 訂單後端

| DDD 概念 | 教科書 | 訂單後端的做法 | 評語 |
|---|---|---|---|
| Ubiquitous Language | 全 codebase 用業務語言 | entity 級有、金流層沒有 | 金流層沿用 Stripe 的詞，自己包一層改名的維護成本太高 |
...
| Domain Event | 業務事件用 event 傳遞 | `OrderPlaced` 等 | 有多個下游要接才發事件，只有一個就直接呼叫 |
...
一開始會想當然全部照做，但實務久了就知道哪些不好用，然後拿掉特定幾條，就像寫 React 久了會說「少用 useEffect」。每個不做的決定，背後都有一個問題：

- 「這個 invariant 真的存在嗎？沒有就不做 aggregate」
- 「這個 subdomain 真的會分裂嗎？不會就不拆 BC」
- 「這個 abstraction 真的會被替換嗎？會才做 ACL」

DDD 實務的共識是，strategic design（bounded context、ubiquitous language，決定 model 的邊界跟用詞）要留；最常被拿掉的是 tactical design（aggregate、domain event 這些 code 層級的 pattern）。

## 寫個 ADR 吧

不做 Aggregate，如果沒寫下為什麼，將來有人想「Customer/Address 這應該做 aggregate 啊」，沒有文件能告訴他「我們討論過，這對沒有 invariant，刻意不做」，下一個剛讀完 Evans 的人就會把 aggregate 加回來。
...
Decision:
不做。Customer 跟 Address 平等，兩個都有獨立的 Repository。
