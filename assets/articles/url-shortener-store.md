---
title: "20 億筆短網址對應，一台 DB 就存得下，為什麼還要 sharding"
...
# 20 億筆短網址對應，一台 DB 就存得下，為什麼還要 sharding

[上一篇](article://url-shortener-key)處理了 key 怎麼產生。現在有幾十億筆 short_key → long_url 的 mapping。每天新增 100 萬筆，存五年大約 20 億筆。
...
12 QPS 的寫入，單機 MySQL 綽綽有餘。6,000 QPS 的讀取，單機也能處理。但要是服務成長到每天 1 億個新 URL，尖峰讀 QPS 就是 60 萬，單機就處理不了了。

---

## 第一步：加 Cache

Read-heavy 系統的標準做法是在 DB 前面放 Redis。
...
3. **Value 小。** 一筆 mapping 就是一個 string（短 key）對一個 string（URL）。Redis 可以存幾億筆。
...
短網址本身有 `expires_at`。Cache 的 TTL 不需要對齊 URL 的過期時間，因為 cache TTL 是為了控制記憶體用量。
...
已過期的 URL（`expires_at < now()`）在 DB 查到後直接回 404，不寫入 cache，免得已過期的 URL 佔掉 cache 空間。

---

## 第二步：DB Sharding

Cache 回應了 80% 以上的讀取。DB 本身 20 億筆、500 多 GB，單機就能存。會走到 sharding，通常是因為服務成長之後，進到 DB 的讀寫量超過單機上限。單表太大讓 backup 跟維護要花很久，也是原因。
...
讀和寫用同一個 hash function，所以一定會到同一台。

### Consistent Hashing

`hash % N` 的問題：加一台機器，N 變了，大量 key 要搬家。

Consistent hashing 把 hash 空間變成環（詳見 [consistent-hashing](chunk://consistent-hashing) chunk）。加一台機器只影響環上相鄰的一小段，搬遷量從大約 80% 降到大約 1/N（下面的例子是 20%）。
...
>
> 這也算 **cache avalanche（快取雪崩）**。[另一篇](article://cache-hot-key)講的雪崩是大量 key 同時過期，Redis 整台掛掉的後果一樣，所有 request 都進到 DB。DB 本來只處理 20% 的量，突然變成 100%，也跟著掛。
>
> 三層防禦，遞進關係：
>
> 1. **Redis HA**（Sentinel / Cluster）— 讓 Redis 根本不掛。Master 掛了自動 failover promote follower。
> 2. **Local cache**（in-process LRU）— Redis 掛了，application 記憶體裡的 Map 還能回應熱門 key。不走網路，同一個 process 裡直接讀，比 Redis 更快。每台 server 各自一份，不共享。
> 3. **Circuit breaker** — DB 錯誤率升高時自動斷路，直接回 503，不再把 request 送到 DB。定時放幾個 request 試探，DB 恢復了再合路。從 application 角度看「我打 DB 有沒有正常回」（timeout、connection refused、error），不需要看 DB 內部的 CPU / 記憶體指標。
>
> ```text
...
> ```
>
> Redis 掛了，服務變慢但還能回應。DB 掛了，資料讀不到也寫不了，服務就停了。所以 circuit breaker 保護的是 DB，因為 DB 後面沒有其他層可以接手。

---

> **「100 萬人同時點同一個短網址，cache 剛好過期。會發生什麼？」**
>
> **Cache stampede / thundering herd**。100 萬個 request 同時 cache miss，同時去 DB 查同一筆資料，DB 收到同一個 query 100 萬次。
>
> 這用 **Singleflight**（也叫 request coalescing）處理。同一個 key 的重複查詢，只放第一個去查 DB，其他等結果。
>
> ```text
...
> ```
>
> 跟 debounce / throttle 不同：debounce 延遲觸發，throttle 丟掉多餘的 request。Singleflight 不延遲也不丟，第一個立刻查，其他 request 等它查完，共用同一份結果。
>
> Singleflight 合併掉瞬間的重複查詢，查完的結果存進 cache，之後就走正常的 cache hit。

---

> **「shard key 選 short_key。但某個 shard 的 URL 特別熱門，流量是其他 shard 的 10 倍。怎麼處理？」**
>
> Consistent hashing 的 virtual node（一台機器在環上佔好幾個點）處理的是資料分布不均，熱點 key 要另外處理。一個 URL 爆紅，那個 key 不管落在哪個 shard，那台就是 10 倍流量。Resharding 只是把 hot key 搬到另一台，那台又變成 hot shard。
>
> 解法在 cache 層，不是 DB 層。讓流量根本不到 DB：
>
> ```text
> 1. Redis cache → 回應 90% 以上的讀取（預設就有）
> 2. Local cache（in-process LRU）→ hot key 連 Redis 都不用查。LRU 容量自己設，例如 10,000 筆（每筆 key 7 bytes + URL ~200 bytes + overhead ~100 bytes ≈ 300 bytes，10,000 筆 ≈ 3 MB）。大部分流量集中在少數熱門 URL（80/20 法則），不需要存很多筆就能涵蓋大部分 hot key
...
>
> Redis 比 DB 快的原因不只是延遲低 10 倍。Redis 是純 RAM HashMap 查找，沒有 SQL 解析、沒有磁碟 IO。它還讓 DB 少做事：10 萬個 request，Redis 回應 9 萬個，DB 只處理 1 萬個。
...
>
> 這時 mapping 就不再是 immutable。DB 改了，但 Redis 和 local cache 還存著舊值，所以要做 cache invalidation。
>
...
>
> 每次 redirect 都直接寫 DB 的話，DB 處理不了，因為 6,000 QPS 的讀取變成 6,000 QPS 的寫入，而寫入比讀取貴很多。所以把「即時回應」跟「資料記錄」拆開：
>
> ```text
...
> ```
>
> Redirect 只多做兩件毫秒級操作（Redis INCR + Kafka append），不碰 DB。詳見[下一篇](article://url-shortener-read)。
...
> **「不能用 Redis。read-heavy 的 DB 怎麼應付？」**
>
> 兩層替代：
>
> 1. **Read replica** — DB 的 leader-follower replication，讀取送到 follower。加 3 個 read replica，讀取就能分到 3 台 follower 上。短網址的 mapping 不會改，replication lag 不影響正確性。
> 2. **Local cache（in-process LRU）** — application 記憶體裡的 Map，先回應熱門 key，不走網路。
>
> ```text
...
> ```
>
> RDS 的「Multi-AZ」就是自動 failover，「Automated Backups」就是每日快照加上交易 log（PostgreSQL 的 WAL、MySQL 的 binlog）。

---

## 每秒十萬次 redirect 怎麼走

每秒十萬次 redirect 的 read path 怎麼走、click analytics 放在哪裡，在[下一篇](article://url-shortener-read)。
