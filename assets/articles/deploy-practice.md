...
實際部署一個 app 時，Docker、Helm、Terraform、ArgoCD 這四個工具是怎麼接在一起的呢？

底下拿一個前端加兩個後端當例子，從 `git push` 一路走到三個環境的 Pod 跑起來。
...
這個例子把 app code 和 k8s config 放在兩個 repo。
...
分開之後，ArgoCD 只 watch config-repo，所以 app-repo 裡的 code 改動跟 CI build 都不會觸發 sync。

---

## Helm：三層 values 覆蓋

Helm chart 的 `values.yaml` 是基礎預設值，每個環境再用自己的 values 檔覆蓋其中一部分。
...
prod 跨多個 region 部署時，會再加第三層 region 覆蓋。
...
k8s 的 CoreDNS 會把 `api-server` 解析成完整的 FQDN（Fully Qualified Domain Name）：
...
`helm.valueFiles` 不影響監控範圍。path 底下任何檔案變了都會觸發 sync，valueFiles 只決定 render 時用哪幾份值。
...
`charts/frontend/` 有任何變更，兩個環境都會 sync。但兩邊用的 values 不同，render 出來的 YAML 也不同。適合小團隊，config 都在 main branch。
...
CI build 並 push image 之後，再更新 config-repo 的 image tag。它不碰 cluster，所以不需要 kubeconfig。
...
**Progressive Delivery**：先放 5% 流量到新版，metrics 正常再慢慢放大，不一次全換。這種做法叫 canary deploy，ArgoCD 搭配 Argo Rollouts 可以做到。

**Secret 管理**：這篇的 values.yaml 裡沒有放 secret。實務上密碼和 API key 要用 Sealed Secrets 或 External Secrets Operator，不能明文放 Git，做法在 [GitOps 那篇](article://gitops-deep)。

**Multi-cluster**：這篇假設所有環境在同一個 cluster 的不同 namespace。大型組織會用不同的 cluster。ArgoCD 的 ApplicationSet 用一份模板產生多個 Application，每個 cluster 一個。
