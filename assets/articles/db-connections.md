...
slug: db-connections
subtitle: "PostgreSQL 每條連線是一個 process，開太多整台就慢下來。中間放 PgBouncer，大部分 Web 應用用 transaction mode，需要 SET、PREPARE、LISTEN/NOTIFY 的服務改用 session mode。"
chapter: "buffer"
...
PostgreSQL 每收到一條連線，就 fork 一個新的 OS process。每個 process 有自己獨立的記憶體空間，實測每條大約 1 到 8 MB，看 huge_pages 有沒有開。

```text
100 條連線進 PostgreSQL：
  → fork 100 個 process
  → 每個 process 獨立 address space，約 1-8 MB
  → 光是連線就用掉幾百 MB RAM
  → OS 要在 100 個 process 之間做 context switch
```

為什麼有 context switch？一台機器的 CPU 核心數有限（通常 4-16 核），但同時有 100 個 process 要執行 query。OS 必須不斷切換：暫停 process A、儲存它的 register 和 memory mapping、載入 process B 的狀態、讓 B 跑一小段、再換下一個。這就是 context switch。Process 越多，CPU 花在「切換」的時間越多，花在「執行 query」的時間越少。就算 100 個 process 跑的 code 完全一樣，每個 process 還是有自己獨立的 address space，OS 切換時必須換掉 page table，TLB（CPU 裡位址轉換的快取）也要跟著清掉，這比 thread 之間切換貴得多。
...
所以 MySQL 開幾千條連線問題不大，PostgreSQL 開幾百條就變慢。

### 為什麼不直接調大 max_connections

PostgreSQL 預設 `max_connections = 100`。直覺是調大就好，但效果遞減。

100 個 process 時，每條 query 排隊拿 lock、搶 CPU 的等待還可以接受。拉到 500 個 process，同一條 query 的 latency 可能從 2ms 增加到 8ms，因為 OS 花更多時間在 process 之間切換，而不是執行 query。連線數多了五倍，throughput 反而下降。
...
## ORM 不知道中間多了 PgBouncer

PgBouncer 接手之後，ORM 的連線字串從 `host=db.internal port=5432` 變成 `host=pgbouncer.internal port=6432`（PgBouncer 的預設 port）。改一行 config，其他什麼都不用動。

ORM 或 driver 連線字串裡的 pool size（例如 `?pool_size=20`）控制的是 ORM 到 PgBouncer 的「假連線」數量，不是 PgBouncer 到 PostgreSQL 的「真連線」數量。PgBouncer 有自己的 `default_pool_size` 設定，決定它維護多少條真的 DB 連線。ORM 這邊開了 20 條連線，PgBouncer 在背後可能只用 5 條真連線輪流服務。

ORM 無法分辨連到的是 PgBouncer 還是 PostgreSQL。連線字串能連上，它就把這條連線當成直連的 DB session 在用。
...
但 PgBouncer 在背後換了真連線，ORM 不知道，不會重新 SET。於是 `search_path` 回到預設值，query 查到錯的 schema。這個過程不會報錯，只有結果是錯的。

---

## 三種 Pooling Mode

PgBouncer 一定會換連線，pooling mode 決定什麼時候換。
...
一個 HTTP request 通常是一個 transaction，處理完連線馬上釋放。池的利用率高。但跨 transaction 的 session state 全部丟失：`SET` 的變數、`PREPARE` 的 statement、`LISTEN/NOTIFY` 的訂閱（連線保持開著，等 server 推通知）。

PgBouncer 新版有 `max_prepared_statements` 設定，會在 transaction mode 追蹤 driver 在 protocol 層送的 named prepared statement，但用 SQL 寫的 `PREPARE` 還是會丟。
...
`SET timezone` 什麼時候需要？PostgreSQL 內部存的 timestamp 是 UTC。`SET timezone = 'Asia/Taipei'` 讓 server 在回傳結果時自動把 UTC 轉成台北時間。多租戶 SaaS 裡，每個租戶可能在不同時區，server 端轉比 client 端轉更統一。要是資料只存 `timestamptz`、時區轉換都在 application 層做，就不需要 SET timezone，transaction mode 也就不會遇到這個問題。
...
### 預設用 transaction mode，用到 session 狀態才換 session mode
...
大部分 Web 應用選 Transaction mode。一個 request 通常就是一個 transaction，所以換連線很少影響到它，池利用率又比 session mode 高。如果 application code 沒有用 `SET`、`PREPARE`、`LISTEN/NOTIFY`，transaction mode 就是安全的預設選擇。

需要 `PREPARE`、`SET`、`LISTEN/NOTIFY` 的場景用 Session mode。換 mode 之前要盤點：application code 和 ORM 有沒有依賴 session-level state。有的話，不能用 transaction mode，或者要在每個 transaction 開頭補 SET。
...
開發期直連 PostgreSQL，沒有 PgBouncer，不需要想 mode。上線後流量成長，連線數接近 `max_connections`，開始拒絕連線。這時候才加 PgBouncer，預設 transaction mode，因為文件這樣寫，大部分教學也這樣推薦。

加完之後一切正常，直到某天多租戶的 `search_path` 偶爾打到錯的 schema，或是 `LISTEN/NOTIFY` 的通知莫名收不到。問題不在 application code，是 PgBouncer 在背後換了連線。

把一條真連線想成一張辦公桌，`SET search_path`、`SET timezone` 是調好的椅子高度。Transaction mode 是輪流坐：每個 transaction 坐到剛好空出來的那張桌子，做完還回去。上一個 transaction 在 #7 桌調好椅子，這個 transaction 坐到 #3，椅子是預設高度。原本的設定還在 #7 那張桌子上，也就是還在真連線 #7 上。
...
回到桌子：路線 A 是每次坐下都先重調一次椅子（每個 transaction 開頭補 SET），不管坐到哪張，調好再用。路線 B 是乾脆要一張自己的桌子坐一整天（session mode），椅子調一次就一直在，代價是這張桌子一直被占著，別人能用的就少了。
...
Statement mode 幾乎不會出現在這條路上，因為它不能用 transaction，適用的場景很少。
...
|---|---|---|
| Supabase | Supavisor（共用）或 PgBouncer（專屬） | port 6543 是 Transaction mode |
...
```text
port 5432 → 直連 PostgreSQL，或經過 Supavisor 的 Session mode（能用 PREPARE）
port 6543 → 經過 pooler 的 Transaction mode（連線池）
```

一般 API 用 6543（pooling），需要 migration 或 PREPARE 用 5432。
