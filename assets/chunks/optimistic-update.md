...
「Optimistic」這個名字來自 concurrency control 的 **optimistic / pessimistic locking**：

- **悲觀（pessimistic）**：假設衝突一定會發生，動手前先上鎖（`SELECT ... FOR UPDATE`），別人只能等鎖放開。對應到 UI，就是上面那個 baseline：**等 server 確認了才改畫面**，白心一直轉到 API 回來。
- **樂觀（optimistic）**：假設衝突很少，不鎖，改完要 commit 時才用 version / ETag 檢查有沒有別人先改過，被改過就 retry。對應到 UI，就是先改畫面、賭它會成功，失敗才 rollback。

所以 optimistic update 就是把樂觀鎖「先做，失敗再復原」的做法用在 UI 上。
...
optimistic update 只發生在**前端**，server-side 不知道。Server 該做的還是要做：

- Server **不能**因為 client 是 optimistic 就少做驗證
- Race condition（兩個 device 同時改）由 server 處理：用「動作」API（POST add / DELETE remove）而不是「整包覆寫」（PUT 整個 array）
