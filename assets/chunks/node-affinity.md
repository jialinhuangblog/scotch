...
這條規則為什麼等於「zone-a 已經有 `app: api` 的 Pod，就不能再放進 zone-a」：
...
Scheduler 檢查每台候選 node 時，先看它的 zone label 值，再看整個 zone 裡有沒有任何 node 跑著 `app: api` 的 Pod。有，這台 node 就被 filter 掉。所以 zone-a 只要有一個 `app: api`，zone-a 的**每一台** node 都不會通過 filter，下一個 Pod 只能去 zone-b、zone-c。
