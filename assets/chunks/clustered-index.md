...
index 跟 B+ Tree 的基礎在 [index-basics](chunk://index-basics)。

磁碟讀取的瓶頸在 seek，找到位置的時間遠比讀資料本身慢。如果相關的資料散落在磁碟各處，每筆都要 seek 一次。如果相關的資料排在一起，一次 seek 讀一整個 page，就取得幾百筆。

Clustered index 就是讓資料按某個 key 的順序排列存在磁碟上。範圍查詢（`WHERE id BETWEEN 100 AND 200`）變成連續讀取，不用在磁碟上來回 seek。

## 定義

Clustered index 要同時滿足：

1. **物理排序**：資料在磁碟上的實際排列順序由某個 key 決定
2. **資料存在 leaf 裡**：B+ Tree 的 leaf page 直接裝整筆 row，查到 leaf 就取得資料
3. **一張表最多一個**：資料只能按一種順序實體排列

選 clustered index 等於決定哪個查詢模式最重要，按 `id` 排就無法同時按 `created_at` 排。

Clustered index 定義了整張表的物理結構。InnoDB 的資料一定存在某個 B+ Tree 裡，所以每張表都有一個 clustered index。沒宣告 PK 時，InnoDB 先找第一個欄位全是 NOT NULL 的 UNIQUE index 當 clustered index；連這種 index 都沒有，才建一個隱藏的 row ID（`GEN_CLUST_INDEX`）。
...
這裡的 page 跟 API pagination（分頁）無關。
...
所有 secondary index（例如 email 上的 index）的 leaf 存的是 PK 值。查 secondary index 後還要用 PK 回 clustered index 再查詢一次。
...
沒有 clustered index，也無法手動建立，因為 PostgreSQL 的資料存在 heap，index 是另外的結構，兩者永遠分開。Clustered index 的前提是「資料存在 index 的 leaf 裡」，heap model 不符合這個前提。

根本原因是 [MVCC](chunk://mvcc) 的實作方式。PostgreSQL 的 UPDATE 會寫新版本到 heap 任意有空間的位置，舊版本留在原地等 VACUUM。如果資料要按 PK 物理排序，新版本就不能隨便塞，會跟 MVCC 的 append-anywhere 模型衝突。InnoDB 可以有 clustered index，是因為它的 MVCC 把舊版本搬到 undo log，leaf page 永遠只有最新資料，不會打亂 B+ Tree 的物理順序。
...
所有 index（包括 PK）都是 secondary。查 index 之後通常還要回 heap 取得資料，多一步。例外是 index-only scan：要的欄位全在 index 裡，而且那個 heap page 在 visibility map 上標成 all-visible，就不必回 heap。
...
資料儲存方式會連帶決定 UPDATE 行為、舊版本放哪、怎麼清理。InnoDB 跟 PostgreSQL 的對照：
...
**PostgreSQL 從 no-overwrite 哲學往下推**：UPDATE 永遠寫新版本 → 新版本塞到 heap 任意位置 → heap 沒有物理順序 → 無法做 clustered index。
...
clustered index 的好處能不能用上，跟 PK 選 UUID 還是 incremental 直接相關，寫在 [PK 選型](chunk://primary-key-choice)。
