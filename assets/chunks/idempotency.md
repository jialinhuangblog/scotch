...
> 同一個操作執行一次和執行多次，結果一樣。API 和分散式系統要能安全重試，前提就是冪等。
...
Client 不知道 server 到底有沒有處理，最直接的做法是重送。但要是 server 又扣了 100，用戶就被扣了 200。

分散式系統裡重試很常見。沒有冪等的話，重試就可能重複扣款、重複下單。
...
API 場景是 client 主動帶 idempotency key。Kafka 場景不一樣：consumer 自己從訊息內容裡挑一個欄位（例如 order_id）當 dedup key，處理過的就跳過。Kafka 的 exactly-once 語意背後也是同一個原理：producer 帶 sequence number，broker 發現重複的 sequence 就丟棄。不管哪種，都是讓接收端記得「這個我已經處理過」。
