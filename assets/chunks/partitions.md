...
> 一個 topic 的訊息量大到一台機器處理不了，怎麼分散又不打亂順序？
...
訊息進來時，producer 決定丟哪個 partition：給了 key 就用 `hash(key) % partition 數`。沒給 key 的話，舊版 Kafka 是輪流放，2.4 之後改成先把一整批訊息放進同一個 partition，滿了再換下一個（sticky partitioner）。同一個 key 永遠落在同一個 partition。

被切開的是「訊息落在哪一段」，不是單獨一則訊息。每則訊息（一筆訂單、一個事件）都是完整的一筆，partition 只決定它分到哪一堆。所以 consumer 分著讀，讀到的是不同的整筆訊息，不是同一筆的片段。

像超市結帳：一個收銀台（單 partition）會排很久，開十個收銀台（十個 partition）就能同時結十個人。但同一位客人跟他的購物車一定走同一個收銀台（同 key 同 partition），不會被拆開。
...
## partition 同時決定順序跟平行度

**順序**：同一個 partition 內嚴格有序，跨 partition 沒有全域順序。要保證某一組訊息的先後（例如同一個 user 的事件），就用同一個 key 讓它們進同一個 partition。

**平行度**：一個 [consumer group](chunk://consumer-groups) 裡，每個 partition 同時只給一個 consumer 讀。所以 consumer 再多，能一起讀的數量上限也是 partition 數。10 個 partition 最多 10 個 consumer 同時讀，第 11 個只能閒著。

## 代價

partition 數量不好改。加 partition 會改變 `hash(key) % N` 的對應，同一個 key 之後落到別的 partition，前後的訊息分在兩個 partition，順序就無法保證。所以一開始就要估夠。
