...
title: "B Tree"
slug: b-tree
brief: "為讀取優化的 index 結構，Postgres 和 MySQL 都用它。"
date: 2026-03-15
updated: 2026-07-20
revisions: 4
...
「往下展開 1000」只發生在 internal 層。Leaf 是終點，不再展開。
...
磁碟一次 seek 約 10ms。27 次 = 270ms，3 次 = 30ms。fan-out 從 2 拉到 1000，層數就少了 9 倍，這就是 B+ Tree 比 BST 家族好用的根本原因。Fan-out 能做到 1000，是因為 internal node 只存 key 不存 data，entry 夠小，一個 page 才放得下 1000 個。
...
容量變成原本的 1/12，但「3-4 層涵蓋幾十億筆」的結論還是成立。設計表時把 TEXT / BLOB 存到別的地方、主表只放指標，每筆 row 變小，一片 leaf 放得下更多筆，同樣筆數需要的層數可能少一層，每次查詢就少一次 IO。
...
B+ Tree 的好處：internal node 不存 data，一個 page 能塞更多 key，樹更矮，IO 更少。Leaf node 之間有指標串連，range query（`WHERE id BETWEEN 10 AND 50`）沿著 linked list 掃描就好，不用回 root。
...
page split 的代價：一次寫可能變成多次寫（leaf 一次 + parent 一次，若 parent 也滿還會往上連鎖）。Random write 也難以避免，因為 INSERT 到中間的 leaf 需要 random IO，UUID PK 受的影響特別明顯（見 [clustered-index](chunk://clustered-index)）。

反過來犧牲讀取、換取寫入速度的結構是 [lsm-tree](chunk://lsm-tree)。
