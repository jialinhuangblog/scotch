...
[Search inverted index](chunk://search-inverted-index) 講原理，ES 用 Lucene 做成可以放上正式環境的 engine。
...
Postgres 要做到這套，得用內建的全文搜尋（FTS，full-text search）、PostGIS、多個 GIN／GiST index（Postgres 給全文跟地理資料用的兩種 index 類型），再加 GROUP BY 算 facets。能做，但 query 複雜、optimizer 不見得選對 plan。ES 一個 request 就能完成。
...
- **不是 transaction store**：寫入是 near-real-time，refresh 預設 1 秒。寫完馬上讀可能還沒看到。每次 refresh 產生一個新的 Lucene segment，segment 寫完就不再修改，背景再用 [merge](chunk://compaction) 把小 segment 合併掉。[LSM Tree](chunk://lsm-tree) 需要 compaction 也是同一個原因：資料只能 append、不能原地修改，就需要背景合併。合併時，詞的索引（term dictionary）要用 k-way merge 重新排序合併，原始文件內容（stored fields）在來源 segment 沒有刪除時可以直接整塊複製。
- **Schema 一旦定型，改 mapping 要 reindex**：欄位 type、analyzer 改了就得重建整個 index。
- **記憶體用量大**：field data、cache 都常駐 heap。**容量規劃比 Postgres 講究**。
- **不適合主交易**：Booking、payment 走 Postgres，搜尋走 ES。

## 什麼時候用

- 文字 search 要 relevance score → ES（Postgres FTS 也行但較陽春）
- 地理 + filter + facets 要在一個 query 做完 → ES
