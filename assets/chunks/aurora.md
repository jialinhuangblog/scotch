...
title: "Aurora"
slug: aurora
brief: "Aurora 計算跟 storage 分兩層，計算層用 leader-follower，storage 層用 quorum。"
date: 2026-03-26
---
...
Aurora 把 DB 拆成兩層：計算層（SQL 處理）和 storage 層（資料存放）。傳統 DB 的計算跟 storage 在同一台機器上。拆開之後，兩層可以分開擴展。
...
傳統 MySQL replication 是每個節點各存一份完整資料，靠 binlog 同步（PostgreSQL 則是 WAL streaming）。Aurora 的所有節點共用同一層 distributed storage。

```text
傳統 MySQL：
  Leader  [自己的 SSD] → binlog → Follower [自己的 SSD]
  每個節點各一份，複製有 lag
...
Aurora：
  ...
  Reader 2 ─┘
  不用複製資料，replica lag 通常遠低於 100ms
```
...
一個 AZ 整個掛了（丟 2 份），還剩 4 份，寫入照常。壞掉的那幾份會自動修復，不影響其他塊。
...
Writer 負責寫入，Reader 負責讀取。因為共用同一層 storage，加 Reader 不需要複製整份資料。
...
## 應用程式連的是 DNS endpoint

Aurora 提供兩個 endpoint：
