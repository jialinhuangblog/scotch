...
slug: http2
brief: "一條 TCP 跑多個 stream，frame 交錯送。握手只做一次，但 TCP 層丟一個包，所有 stream 都要等重傳。"
date: 2026-04-28
updated: 2026-07-20
revisions: 2
...
50 個資源 = 50 個 stream，全部走同一條 TCP。**握手只做一次**。CDN、API gateway 都大量採用。
...
HTTP/1.1 反而沒這問題，6 條 TCP 各自獨立，丟包只影響那一條。

所以**在高丟包網路（行動網路、弱 Wi-Fi）下，HTTP/2 可能比 HTTP/1.1 慢**。[HTTP/3 改用 QUIC](chunk://http3-quic)，就是為了解決這個問題。
