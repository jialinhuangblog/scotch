...
k8s 不執行命令。宣告「我要 3 個 nginx Pod」之後，controller 負責讓現實符合宣告。這個持續比對、修正的過程就是 reconciliation。
...
這個比對不是一次性的。Pod 掛了，下一輪 loop 又會發現 actual = 2，再補一個。

## 為什麼不用命令式

命令式：「啟動一個 Pod」。要是網路斷了、命令沒送到，Pod 就不會啟動，也無從得知最終狀態是什麼。

聲明式 + reconciliation：「我要 3 個 Pod」寫進 etcd。不管中間發生什麼（網路斷、node 掛、container crash），controller 都會持續把狀態修正回 3。
...
Deployment controller 的 actual 不是 Pod，是 ReplicaSet。它只管 ReplicaSet，ReplicaSet 才管 Pod。每一層都只跑自己那個 loop，層層委派下去。

自訂的 controller（operator）也跑同一個 pattern。Argo CD 的 sync loop、cert-manager 的憑證更新，全是 reconciliation。

---

這個 loop 讀的是當前狀態，不是等事件通知。為什麼這樣選、漏掉某次觀察為什麼沒關係，寫在 [Edge vs Level Triggered](chunk://edge-vs-level-triggered)。
