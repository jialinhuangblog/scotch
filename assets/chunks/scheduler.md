...
其他 filter 條件還有 [taint 沒有對應 toleration](chunk://taint-toleration) 跟 [affinity 規則不符合](chunk://node-affinity)，兩個都有獨立的 chunk。

## Score：從通過的 node 中挑最佳

Filter 後可能剩多個 node。Score 階段由多個擴充（scheduler plugin）分別打分，加總之後選最高分的 node。
