...
title: "Bloom Filter"
slug: bloom-filter
brief: "Bloom filter 花極小記憶體判斷「一定不存在」，不存在的直接回答，不用查磁碟。"
date: 2026-03-21
updated: 2026-07-20
revisions: 2
---
...
> 用極少的記憶體判斷「這個 key 一定不在」。用在 LSM-tree、CDN、爬蟲 URL dedup、Chrome 安全瀏覽。

## 問題

LSM-tree 讀取一筆資料時，不知道它在哪個 SSTable。最直接的方法是從新到舊把每個 SSTable 都打開查一遍。10 個 SSTable 就是 10 次磁碟讀取。如果那筆資料根本不存在，10 次全白跑。
...
Bloom filter 放在 RAM 裡。查一次 bloom filter 是記憶體操作，查 DB 是磁碟 + 網路操作。速度差距：
...
照 10 bit per key 算，1,000 萬個 key 的 bloom filter 也只佔 12 MB RAM，就能省掉大量不必要的磁碟查詢。
...
**Chrome 安全瀏覽**：Google 維護一份惡意 URL 列表。Chrome 不可能每次開網頁都去 Google 查。早期的 Chrome 把列表壓成 bloom filter 放在本地，「可能是惡意」才去 server 確認，後來改用更省空間的 prefix set。
...
**RedisBloom vs 原生 Redis**：舊版 Redis 沒有 bloom filter，要另外裝 RedisBloom 這個 module（外掛），裝上去之後才多了 `BF.ADD`、`BF.EXISTS` 這些指令。新版的 Redis Open Source 已經直接內建這組指令。好處是 bloom filter 跟著 Redis 一起持久化，重啟不用從 DB 重建。要是 Redis 沒有這組指令，bloom filter 只能放在 application 的記憶體裡，重啟就沒了。Redis 怎麼把 RAM 裡的資料存到磁碟（RDB / AOF），寫在 [Redis Persistence](chunk://redis-persistence)。
