...
> 分散式系統的共識演算法，1989 年由 Leslie Lamport 提出，比 Raft 早了二十多年。
...
1. **多個 Proposer 同時提案**：A 提 N=1，B 提 N=2，A 的 Accept 被拒絕（Acceptor 已承諾 N=2），A 要用更大的 N 再來。兩邊可能輪流用更大的編號蓋過對方，一直達不成共識（livelock）。
...
|---|---|---|
| 誰能提案 | 任何節點 | 只有 leader |
| 衝突 | 多個 Proposer 互搶 | 不會，只有 leader 提案 |
| 可理解性 | 出了名的難懂 | 設計目標就是讓人能理解 |

實務上新系統多半選 Raft（etcd、CockroachDB、TiKV）。Paxos 主要用在 Google 內部系統（Chubby、Spanner）。
