...
CDN 是 read-through cache 的大規模版本。邊緣節點 miss 了，就回源站取得內容並快取，下次同一個 request 直接由節點回應。Application 不需要知道 CDN 的存在。

## Pull vs Push

**Pull CDN**：第一個用戶 request 到邊緣節點，miss，節點回源站取得內容並快取。之後的 request 命中。這是大部分 CDN 的預設模式。
...
缺點：第一個用戶一定慢。流量低的內容可能 TTL 過期後又要重新回源站取得。

**Push CDN**：由源站主動把內容推到邊緣節點，不等 request 進來。適合大型靜態資源（影片、firmware），或發布前就知道流量會暴增的內容。

大部分場景用 pull，能事先預測流量的時候才值得用 push。
...
Invalidation 是 CDN 最麻煩的部分。內容更新之後，所有邊緣節點上的舊快取要怎麼清呢？
