...
把資料存取從 application code 中抽離。**domain 層只看到「介面」**，**infrastructure 層提供具體實作**。中間用 DI（Dependency Injection）container 接起來，由它在啟動時決定注入哪個實作。
...
export class PrismaConversationRepository implements ConversationRepository {
  ...
  async getById(id: string) {
    ...
    return row ? toDomain(row) : null;     // prisma model → domain 的 Conversation
  }
...
// 沒有 repository：SQL / ORM 直接寫在業務邏輯裡
...
ORM（Hibernate / TypeORM / Prisma）本身**不是 Repository pattern**。ORM 提供可以直接呼叫的 CRUD API。Repository 是在 ORM **再上一層**抽象，讓 application 完全不接觸 DB 細節。
