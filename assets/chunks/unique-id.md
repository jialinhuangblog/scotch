...
它的問題是字串形式有 36 字元，當 DB primary key 時 index 會膨脹。而且完全隨機、沒有時間順序，B-tree 寫入變成隨機插入，效能差。

UUIDv7 改善了這點：前 48 bit 是 timestamp，保留了時間順序。但長度仍然是 128 bit。

## Snowflake

Twitter 在 2010 年公開的 ID 格式設計，不是套件或服務。各語言自己實作這個演算法（Node: `snowflake-id`、Go: `bwmarrin/snowflake`、Java: `Hutool IdWorker`）。核心就是一個 function：輸入 timestamp + machine ID，輸出 64-bit 整數，可以直接當 `BIGINT` primary key。
...
ID 自帶時間排序：timestamp 在最高位，晚產生的 ID 一定比早產生的大，所以 B-tree index 的寫入永遠是 append 到尾端。

## 為什麼 Snowflake 需要 Machine ID

64 bit 的空間不夠大，不能像 UUID v4 那樣靠「128-bit 隨機到不會撞」。Snowflake 靠 machine ID 區分不同機器，確保同一毫秒內不同機器產生的 ID 不會重複。
...
「machine」不限於實體機器，K8s 裡通常對應到一個 pod。10 bit 最多容納 1024 個同時產生 ID 的 instance。
