...
借用 HTTP 握手是為了**穿過 firewall 跟 proxy**，它們認得 HTTP，但不認得一開頭就不是 HTTP 的連線。升級完成後就不再受 HTTP 規則限制。
...
> 範圍從 2 變 14 是因為長度欄位用 [variable-length encoding](chunk://varint-encoding)（≤125 用 7 bit、≤64KB 多 2 bytes、更大多 8 bytes），加上 client 送出時要額外 4 bytes masking key 防 cache 攻擊。常見的短訊息 overhead 最少。

[HTTP/2](chunk://http2) 下，WebSocket 用 CONNECT 方法走一個 stream（RFC 8441），不再佔住整條 TCP；server / 瀏覽器都支援才會走這條，否則自動降回 HTTP/1.1。

## 代價

**Stateful connection**。每條 WebSocket 綁定一台 server。Server 重啟、scale-out、deploy，連線斷開，client 要重連。Load balancer 要支援 sticky session（同一個 client 固定導到同一台 server），或是用懂 WebSocket 的 L7 load balancer。

**不能用 HTTP 工具鏈**。CDN、cache、API gateway、middleware 大多基於 HTTP request/response model，WebSocket 不是 request/response，不能直接套。

**連線數成本**。10 萬條 WebSocket = 10 萬條 TCP 連線常駐記憶體。每條幾 KB，但 OS file descriptor 上限要調。
...
|  | 場景 |
|---|---|
| [Short polling](chunk://long-polling) | 量低、要最簡單的相容做法 |
...
10 萬條 WebSocket 不該由 application server 直接管。常見的做法是在中間加一層 WebSocket Gateway：
