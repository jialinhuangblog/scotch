...
> 「找 5 公里內的餐廳」這個 query，DB 不能只看經緯度兩個 float 兩兩比較。怎麼把地理位置變成可索引的字串？
...
```text
u0      → 一大片西歐
u09t    → 巴黎周邊（約 39km × 20km）
u09tunq → 巴黎某個街區（約 150m × 150m）
```
...
Geohash 是字串，可以建 B-tree index。「巴黎附近」就變成 `WHERE geohash LIKE 'u09t%'`。
...
(網格邊界)
  ezzpb... | ezzpc...
        ↓
   兩個點實際距離 50m，prefix 卻從第五個字就不同
   要是剛好落在更大的格線兩側，連第一個字都可能不同
```

所以查詢時**取目標點 + 周圍 8 格 = 9 格的 prefix**，全部查出來，再用實際距離過濾。Redis 的 GEOSEARCH、ES 的 geo_distance 內部都這樣做。
...
geo 查詢本身就是主要的 search workload，就用 ES；只是輔助查詢的話，用 Redis 或 PostGIS。

## Marker Cluster

地圖縮到全國 view，一個 viewport 有 100,000 個點，前端無法全部畫出來。這時按 zoom level 對應的 geohash precision 聚合，**每格回一個 cluster 中心 + count**：
...
ES 的 `geohash_grid` aggregation 直接做這種聚合。
