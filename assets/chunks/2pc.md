...
slug: 2pc
brief: "兩階段提交。最大的弱點是 coordinator 在兩個階段之間掛掉，所有節點都鎖著資料等它回來。"
date: 2026-03-16
---
...
問題出在 users 多到一台裝不下、被 [sharding](chunk://sharding) 拆到多台機器之後。A 的帳戶可能落在 DB-1、B 的落在 DB-2，邏輯上同一張 users 表，物理上不同機器。扣 A 的錢（DB-1）、加 B 的錢（DB-2）要同時成功或同時失敗，但兩台機器各跑各的 transaction，DB-1 不知道 DB-2 那邊成功還是失敗。
...
這是 2PC 最大的弱點。Coordinator 如果在 Phase 1 之後、Phase 2 之前掛了：

- 節點已經回了 YES，資料鎖住了，等 Coordinator 的指令
- Coordinator 不回來，節點不知道該 commit 還是 abort
- 資料一直鎖著，其他要動這幾筆 row 的 transaction 都得等
