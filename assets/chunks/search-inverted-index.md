...
brief: "把詞對應到文件。全文搜尋用這個資料結構找出含有某個詞的文件。"
date: 2026-03-09
updated: 2026-04-08
revisions: 1
---
...
最直接的做法是逐篇讀過一百萬篇，每篇檢查有沒有這兩個詞，一次搜尋要幾十秒。
...
搜尋「distributed」要讀過所有文件，看哪些包含這個詞。文件越多越慢。
...
搜尋「distributed」查一次表就取得 [doc1, doc3]，不用讀過每篇文件。
...
Inverted index 擅長全文搜尋：「包含某個詞的所有文件」。B-tree 做不到，除非把每個詞都當 key 建 index，而且它不支援分詞，也不支援相關性排序。
