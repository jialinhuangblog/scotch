...
協議跟檔案格式裡到處是「大多時候很小、偶爾很大」的值，要怎麼用**最少的 bytes** 存它們呢？這種值（訊息長度、欄位數量、一個字元佔幾 bytes），範圍可能從 2 一路到上 GB。為了裝得下最大值就用固定大寬度，結果小值全在浪費；變長編碼就是來解這件事的。protobuf、UTF-8、WebSocket frame、HPACK 都靠它省 bytes。
...
假設要設計一個 protocol，header 裡有「payload 長度」欄位。
...
**欄位寬度跟著值的大小走**，短的編碼到最小，長的也允許，只是多一點 overhead。
...
小數字（0-127）只占 1 byte，大數字依規模延伸。protobuf 一條 message 裡欄位 tag 跟 length 全用這套。
...
continuation bytes 開頭都是 `10`，自帶**自同步**：從 stream 中間開始讀，也能找到字元邊界。
...
三種實作都是 **protocol designer 對資料分佈下的判斷**：
