...
常見的 storage engine 大多是這兩種結構之一：

| | B+ Tree | LSM Tree |
|---|---|---|
| 寫入 | 每筆找到正確位置塞進去（random write） | 先存記憶體，批次寫磁碟（sequential write） |
| 讀取 | 走樹找，3-4 次 IO | 要查 memtable（記憶體裡的寫入緩衝）+ 多個 SSTable（磁碟上排好序的檔案） |
| 適合 | 讀多寫少 | 寫入密集 |
...
| RocksDB | LSM Tree | TiKV、Kafka Streams（CockroachDB 早期也用，後來換成相容的 Pebble） |
| LevelDB | LSM Tree | Google 出的，RocksDB 的前身 |
| Cassandra engine | LSM Tree | Cassandra |

拆開 InnoDB 寫入流程，就是 B+ Tree engine 的標準步驟：Buffer Pool → WAL（Write-Ahead Log）→ Checkpoint → B+ Tree page。這套機制是 B+ Tree engine 共有的，不是 MySQL 獨有。
...
大部分應用從 PostgreSQL 或 MySQL 開始，遇到特定瓶頸再加：
