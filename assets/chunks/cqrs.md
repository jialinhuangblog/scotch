...
一般系統用同一張表讀寫。寫入用正規化的 schema（避免重複），讀取也從這個 schema 讀（可能需要 JOIN）。

問題出在兩端的需求不同。寫入要的是一致性和正確性。讀取要的是速度和方便的結構。一張表很難同時滿足。

CQRS 把寫入導到 command model（正規化的表），讀取導到 query model（反正規化的、預先計算好的 view）。
...
Query model 由 command model 的變更事件驅動更新。通常透過 message queue 非同步更新。
...
- 已有 event-driven 架構，query model 直接訂閱既有的事件就能更新
