...
slug: covering-index
brief: "查詢要的欄位剛好都在 index 裡就不用回 table；composite index 要從最左欄位開始用，查詢沒帶最左欄位就走不了這個 index。"
date: 2026-07-20
---
...
實務上把「過濾性最強的欄位」放最左。`tenant_id` 一個值就能把資料縮小到一個 tenant 的範圍，`created_at` 在這個範圍內再排序。反過來放成 `(created_at, tenant_id)` 的話，只給 `tenant_id` 的查詢就沒有最左欄位，走不了這個 index。

## 代價

每次 INSERT / UPDATE / DELETE，所有相關 index 都要跟著更新。三個 index 就更新三次 B+ Tree。想靠加欄位讓更多查詢 covering，寫入就更慢。
