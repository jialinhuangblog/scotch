...
好處：app code 控制要送什麼 event 出去（[domain event](chunk://domain-event)），不直接暴露 DB schema 給下游。代價是多一張表，已送出的 rows 還要定期清理。
...
下游只要一份資料副本（search index、cache invalidation、進 data warehouse），直接 CDC 業務表就好，outbox 反而多此一舉。跨服務的整合事件才需要 outbox。

1. **意圖不見了。** 「下單」一個 transaction 碰五張表，WAL 流出來是五筆互不相干的 row change，下游要自己拼回「有人下單了」。outbox 裡就是一筆 `OrderPlaced`，事件本身就是意圖。
2. **內部 schema 變成對外 API。** 下游直接訂閱 `orders` 表的話，業務表一 rename 欄位或拆表，所有 consumer 會一起故障。outbox 的 payload 是設計過的事件格式，內部怎麼重構都不影響它。
3. **過濾與隱私。** 業務表整 row 流出去，包括成本價、個資這些下游不該看的欄位。outbox 只放刻意要給下游的欄位。

兩條路都靠 WAL 流出 DB。直接 CDC 業務表，下游看到的是一筆筆 row change；走 outbox，下游看到的是設計過的事件。
