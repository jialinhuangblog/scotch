...
最常見的架構。一個 leader（也叫 primary / master）負責所有寫入。Follower（也叫 replica / slave）從 leader 複製資料，只接受讀取。Kafka 也是這種架構：每個 partition 一個 leader，其餘 broker 當 follower。
...
每個 partition 有一個 leader 和多個 follower。ISR 是「目前跟得上 leader」的 follower 清單。Kafka 用 `replica.lag.time.max.ms`（預設 30 秒）判斷：如果一個 follower 超過 30 秒還沒追上 leader 的最新資料，就被移出 ISR。
...
被移出的 follower 不會被刪除。它繼續嘗試從 leader 拉資料追進度。一旦追上（lag 回到門檻內），自動回到 ISR。
