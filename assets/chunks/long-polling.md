...
title: "Long Polling"
slug: long-polling
brief: "Client 發一個 request，server hold 住到有資料才回。模擬 push 最早的做法，相容性最好。"
date: 2026-04-28
...
| Server 寫法 | 要 non-blocking I/O + pub/sub 才能處理這個量 |
| 短暫空窗 | timeout / 重連那幾百毫秒收不到訊息 |

## 為什麼還有人用

[SSE](chunk://sse) 跟 [WebSocket](chunk://websocket) 都比 long polling 有效率，但 long polling **就是 HTTP**：

- 任何 firewall / proxy / load balancer 都能通過
- 不需要 HTTP/1.1 的 chunked、不需要 Upgrade header
- HTTP/1.0 也能用（雖然每輪都重連 TCP）
- 後端任何 HTTP framework 都能寫，沒有 stream API 也行

**Socket.IO 的 fallback 就是 long polling**：先用 long polling 確保能連上，能升級才升級成 WebSocket。企業內網、嚴格 proxy 環境，long polling 是最後的退路。
