...
| MQTT（協議，配 Mosquitto/EMQX） | Pub/Sub | 依設定（可 retain） | IoT、低頻寬裝置 |

## 選型邏輯

**需要 replay**（consumer 掛了要補讀、多個 consumer 各自讀）→ Kafka。訊息是 log，不會因為被讀過就刪掉。

**需要任務只被處理一次**（job queue、email 發送）→ RabbitMQ 或 SQS。訊息被 ack 後刪除，正常情況下只會交給一個 worker。但 SQS standard queue 偶爾還是會重送同一則，所以 worker 仍要做冪等。
...
Kafka 底層是 append-only log，容易跟「logging（記錄系統事件）」混淆。兩個是不同概念：Kafka 是通訊 broker，logging 是 debug 工具。

**MQTT 是協議，不是 broker。** MQTT 只定義訊息怎麼收發，要實際收發訊息，得配 Mosquitto、EMQX 這類 broker。表裡其他項目都是產品，MQTT 是以「協議＋broker」的組合放進來比較的。
