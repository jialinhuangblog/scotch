...
title: "Sharding"
slug: sharding
brief: "資料切到多台機器。Hash 切法讓 range query 要掃描所有 shard，range 切法容易讓最新資料集中在一台。"
date: 2026-03-15
updated: 2026-03-27
revisions: 1
...
一台 DB 處理不了，就把資料分散到多台。每台只存一部分資料，叫一個 shard。
...
所有 sharding 都是 horizontal partition，但不是所有 partition 都是 sharding。同一台機器上的 partition 解決的是查詢效能和資料管理（例如按月份 partition，刪舊資料直接 drop partition）。Sharding 解決的是單機處理不了的量（CPU、磁碟、QPS 超過一台的上限）。
...
問題：`WHERE created_at BETWEEN ...` 這種 range query 要掃描所有 shard，因為 hash 打散了順序。

### Range Sharding

按 key 的範圍分。`id 1–1000` 在 shard 0，`1001–2000` 在 shard 1。Range query 只需要掃描一個或少數幾個 shard。

問題：如果 key 分布不均（例如最新的 id 都落在最後一個 shard），容易產生 hot spot。

## Shard Key 的選擇

shard key 決定資料怎麼分散，也決定哪些查詢只要查一個 shard。選錯了，resharding 的代價巨大。
...
Consistent hashing 能減少搬遷量，加一台時大約只要搬 1/N 的資料。但實務上 resharding 仍然是大工程，需要 double-write、驗證、切換。
...
Hash 分布均勻，但 range query 要掃描全部 shard。Range 保住順序，但最新資料容易集中在最後一個 shard。選 shard key 幾乎是一次性決定，之後改的成本極高。
