...
k8s 只定義 container runtime 跟網路的介面，實作由其他專案提供。

## CRI（Container Runtime Interface）

Kubelet 要啟動 container 時，不直接呼叫 Docker 或 containerd，而是透過 CRI 這個 gRPC 介面去呼叫 runtime。
...
為什麼要多一層呢？因為 k8s 早期綁死 Docker，而 Docker daemon 包了 build、push、network、volume 一大堆功能，k8s 只需要「跑 container」。CRI 把介面抽出來，任何 runtime 只要實作 CRI 就能用。（完整的 container runtime 演進史在 [K8s needed Docker. Then it didn't.](https://jialin00.com/v2/container-runtime-evolution)。）
...
同一個問題，不同環境有不同最佳解。雲端用雲端的 CNI（VPC CNI），bare metal 用 Calico，安全敏感場景用 Kata + Cilium。
