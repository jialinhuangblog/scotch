...
title: "Split Brain"
slug: split-brain
brief: "同時出現兩個 leader，兩邊各自接受寫入，網路恢復後兩邊各有一批對方沒有的寫入。"
date: 2026-03-16
---
...
網路一斷，叢集就分成兩邊。兩邊各自選出自己的 leader，各自接受寫入。等網路恢復，兩邊各有一批對方沒有的寫入。
...
但「收不到 heartbeat」不代表 leader 真的掛了。可能只是網路延遲或暫時斷開。原 leader 還活著、還在接受寫入，另一邊已經選出新 leader。兩個 leader 同時存在，就是 split brain。
...
偵測到可能 split brain 時，直接強制關閉另一台機器（透過 IPMI、PDU 斷電等硬體手段）。舊 leader 斷了電，就不可能繼續寫入。

傳統 HA（High Availability）叢集常用。
...
| Redis Sentinel | Sentinel 投票決定 failover，但 failover 期間有一小段時間可能 split brain |
