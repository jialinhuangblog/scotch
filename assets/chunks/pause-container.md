...
每個 Pod 裡都有一個平常看不到的 container，叫 pause container。它只執行一個 syscall：`pause()`（永遠 sleep）。
...
CRI 只說 `RunPodSandbox()`，意思是「幫這個 Pod 建一個共享環境」，但沒規定這個環境要怎麼建立。

containerd 的實作是：建一個 pause container 來持有那些 namespace。

sandbox 是 CRI 規格上的講法，pause container 是 containerd 的實作。
