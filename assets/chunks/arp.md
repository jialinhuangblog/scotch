...
筆電知道 gateway 的 IP 是 `192.168.1.1`。但 Ethernet frame 的 header 要填 MAC 位址，IP 填不進去。ARP 負責把 IP 翻成 MAC。
...
ARP 只在 LAN 內運作。跨網段的目的地不需要知道對方的 MAC，只需要知道 gateway 的 MAC，封包先送給 gateway，再由 router 往外轉送。
...
- **macOS**：預設 ~20 分鐘沒流量就刪
- **Linux**：大約 15 到 45 秒沒再確認就進入 stale 狀態，要用的時候重新探測；長時間沒用的 entry 會被回收

過期是為了應付 device 下線或 IP 重新分配。ARP cache 是**當下可達的鄰居快照**，不是歷史紀錄。
...
[DHCP](chunk://dhcp) 解決「我還沒有 IP」，ARP 解決「我知道 IP，但不知道 MAC」。接入網路時，DHCP 先給 IP、gateway 跟 DNS，之後每次送封包，ARP 才去找下一跳的 MAC。
