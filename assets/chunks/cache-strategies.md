...
> 系統加了 cache 之後，讀的時候先查 cache 還是 DB？寫的時候先更新哪一邊？
...
下次讀 "user:42" 就直接從 cache 取得，不碰 DB。
...
**純 Redis 做不到 read-through**。Redis 是 key-value store，沒有「miss 時去 DB 取資料」的 callback 機制。
...
把 loader function 註冊給 library 之後，library 會在 miss 時自動呼叫它。從 application code 看像 read-through，實質是 cache-aside 被 library 包起來。

**2. 綁定特定 DB 的託管 cache**（AWS DAX）

DAX 是 DynamoDB 專用的 cache layer，miss 時自動回 DynamoDB 取資料。App 只呼叫 DAX，完全不接觸 DynamoDB。這是真正原生的 read-through。

**3. CDN**

CDN 節點 miss 時自動回源站取得內容，application 不需要知道。

### Redis + MySQL 幾乎都是 cache-aside

平常寫的「先 GET cache，miss 再 query DB，然後 SET cache」就是 cache-aside。Redis 沒有 loader 機制，所有 cache miss 後的邏輯都在 application code 裡。有人把這種模式口語上叫 read-through，嚴格講法是 cache-aside。
...
適合寫多讀少的資料。剛寫入的資料不一定馬上會被讀，沒必要佔 cache 空間。
...
請求沒被 cache 回應、直接進到 DB 時會發生什麼（雪崩、穿透、擊穿），拆在 [Cache Stampede](chunk://cache-stampede)。
