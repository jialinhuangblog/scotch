...
slug: fan-out
brief: "Push 發文時就寫進所有 follower 的 feed，Pull 讀取時才查詢所有 following 的貼文。只用其中一種，到大規模時不是寫入量太大，就是讀取太慢。"
date: 2026-03-21
---
...
```text
               寫入量       讀取速度    名人帳號    普通帳號
Fan-out Write  O(followers)  快         寫入量太大  剛好
...
follower 數超過某個門檻（例如 10 萬），這個帳號就切換成拉模型。

大部分用戶的 feed 已經有 90% 的內容（來自 fan-out on write），只需要額外拉少數幾個名人帳號的貼文。讀的時候快，名人發文時寫入量也不會暴增。

## 背後的基礎設施

Fan-out on write 的 500 次寫入不是同步的。通常是丟進 message queue（Kafka、SQS），由 worker 非同步處理。
...
這些場景的選法跟 feed 一樣，接收者少就推，接收者多就拉，實務上多半混合使用。
